...
- [API 설계를 시작하는 나의 방법](#api-설계를-시작하는-나의-방법)
  ...
  - [협업에 있어서 문서화의 중요성](#협업에-있어서-문서화의-중요성)
  - [목표가 있어야 평가를 할 수 있다](#목표가-있어야-평가를-할-수-있다)
  - [Test Driver](#test-driver)
  - [일을 어디까지 쪼갤거냐](#일을-어디까지-쪼갤거냐)
  - [작업계획을 빠르게 세우고 같이 토론한다](#작업계획을-빠르게-세우고-같이-토론한다)
  - [스크럼](#스크럼)
...
기획 단에서 작성했던 시스템 구성도, 서비스 구성도 등의 여러가지 문서는 다 계속 업데이트 되어야한다.
...
MVP Scenario 하나마다 나와야하는 것이다

Sequence Diagram
또는
Google Docs

어떻게 논리적으로 실수를 미리 감지할 것이냐
이거와 관련된 작업이다

### Test Driver

Test Driver
더미로 나를 부르는 애를 가짜로 만들어 놓는 것이다.
...
Stored Procedure
데이터베이스도 일종의 랭귀지 셋이다.
거기다가 사용자 정의형 function을 만드는 것이다.
(필요하다면)

복잡한 쿼리를 function으로 만드는 것이다.
네트웍을 타고 왔다갔다하지 않기 때문에 빠르다.
...
### 작업계획을 빠르게 세우고 같이 토론한다

작업하다가 이슈가 생기면 혼자 해야할 것임.
그래서 전에 미리 하면서
내가 보기에는 이슈가 있는데 하고 
말하는 것이다.

진행 방향이나 목표를 댓글로 적는 작업은 간단하게 하자, 여기에 너무 많은 시간을 투자하지 말자
그러나 적어두면 글만으로도 팀원들이 토론이 가능하다
